import React from "react";
import "./IntervalViewer.css";

const days = [
  "Sunday",
  "Monday",
  "Tuesday",
  "Wednsday",
  "Thursday",
  "Friday",
  "Saturday"
];

//this is a dropdownlist component to be used in the component AddTime only.
const DropDownList = props => {
  const [timeValue, setTimeValue] = React.useState("0900");
  const [workingHoursList, setWorkingHoursList] = React.useState(
    [
      "0900",
      "0930",
      "1000",
      "1030",
      "1100",
      "1130",
      "1200",
      "1230",
      "1300",
      "1330",
      "1400",
      "1430",
      "1500",
      "1530",
      "1600",
      "1630",
      "1700",
      "1730",
      "1800",
      "1830",
      "1900",
      "1930",
      "2000",
      "2030",
      "2100",
      "2130",
      "2200",
      "2230",
      "2300",
      "2330"
    ].sort()
  );
  const changePick = e => {
    setTimeValue(e.target.value);
  };

  React.useEffect(() => {
    props.setHour(timeValue);
  }, [timeValue]);

  return (
    <div>
      <select onChange={e => changePick(e)} name="hours">
        {workingHoursList.map(halfHour => (
          <option key={halfHour} value={halfHour}>
            {halfHour.slice(0, 2) + ":" + halfHour.slice(-2)}
          </option>
        ))}
      </select>
    </div>
  );
};

//this component consists of 2 DropDo0wnLists and a button to modify the list in the component IntervalViewer
const AddTime = ({ hidden, addSlot, day }) => {
  const [start_hr, setStartTime] = React.useState("0900");
  const [end_hr, setEndTime] = React.useState("0900");
  const week_day = day;
  const executeButtonFunctionToEditTheListInPreviousComponentPlease = () => {
    addSlot({ week_day, start_hr, end_hr });
  };
  return (
    <div hidden={hidden}>
      <DropDownList setHour={setStartTime} />
      <DropDownList setHour={setEndTime} />
      <button
        onClick={() =>
          executeButtonFunctionToEditTheListInPreviousComponentPlease()
        }
      >
        {" "}
        add{" "}
      </button>
    </div>
  );
};

//component that shows all the times the barber works in one specific week day
const IntervalViewer = props => {
  const [editable, setEditable] = React.useState(false);
  const week_day = days[props.day];
  const deleteUrl = "/api/cancelschedulebyday/";
  const [list, setList] = React.useState([]);

  const deleteAndAddTimes = () => {
    if (editable) {
      let modifydata = {};
      modifydata.day = props.day;
      modifydata.list = JSON.parse(JSON.stringify(list));

      //this line deletes all working hours for this day
      fetch("/api/modifyworkingday", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify(modifydata)
      }).catch(err => console.log(err));
    }
    setEditable(!editable);
  };

  React.useEffect(() => {
    fetch("/api/getallhours/" + props.day)
      .then(res => res.json())
      .then(dblist => setList(JSON.parse(JSON.stringify(dblist))))
      .catch(err => console.log(err));
  }, []);

  return (
    <div className="interval-day">
      <div className="booking-header">
        <div className="booking-date">{week_day}</div>
        <AddTime
          addSlot={obj => {
            setList([...list, obj]);
          }}
          day={props.day}
          hidden={!editable}
        />
        <div className="booking-cancel">
          <button className="edit-button" onClick={deleteAndAddTimes}>
            {editable ? "Done" : "Edit"}
          </button>
        </div>
      </div>
      <div className="working-hrs">
        {list.map(item => {
          return (
            <div key={item}>
              {item.start_hr} - {item.end_hr}
              <button
                onClick={() => {
                  setList(
                    list.filter(
                      iterator => iterator.schedule_id !== item.schedule_id
                    )
                  );
                }}
                hidden={!editable}
              >
                delete
              </button>
              <br />
            </div>
          );
        })}
      </div>
    </div>
  );
};

export default IntervalViewer;
